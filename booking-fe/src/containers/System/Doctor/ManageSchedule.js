...
class ManageSchedule extends Component {
    ...
    componentDidMount() {
        ...
        if (userInfo?.roleId === 'R2') {
            this.setState({
                selectedDoctor: {
                    label: `${userInfo.lastName || ''} ${userInfo.firstName || ''}`.trim(),
                    value: userInfo.id
                }
            });
        } else {
    ...
    handleClickBtnTime = (time) => {
        ...
        if (rangeTime && rangeTime.length > 0) {
            rangeTime = rangeTime.map(item => {
                if (item.id === time.id) item.isSelected = !item.isSelected;// true or false
                return item;
            })
            this.setState({
                rangeTime: rangeTime
    ...
    handleSaveSchedule = async () => {// lưu thông tin
        ...
        console.log('check result check saveBulkScheduleDoctor ', res)
    ...
    render() {
        ...
        //  console.log('chuong check state', rangeTime)
        return (
                        ...
                        <div className="col-12 pick-hour-container">
                            {rangeTime && rangeTime.length > 0 &&
                                rangeTime.map((item, index) => {
                                    return (
                                        <button className={item.isSelected === true ? "btn btn-schedule active" : "btn-schedule"}
                                            key={index}
