    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (prevProps.genderRedux !== this.props.genderRedux) {
            ...
            this.setState({
                ...
                gender: arrGenders && arrGenders.length > 0 ? arrGenders[0].keyMap : '' // check length lay phan tu dau tien or not return rong
            })
        }
        if (prevProps.roleRedux !== this.props.roleRedux) {
            let arrRoles = this.props.roleRedux;
            // Filter out 'Patient' (R3) for staff management
            let codeRoles = arrRoles.filter(item => item.keyMap !== 'R3');
            this.setState({
                RoleArr: codeRoles,
                role: codeRoles && codeRoles.length > 0 ? codeRoles[0].keyMap : ''
        ...
        if (prevProps.listUser !== this.props.listUser) { // hàm sét giá trị về 0 khi ấn save
            let arrRoles = this.props.roleRedux;
            let arrGenders = this.props.genderRedux;
            let arrPositions = this.props.positionRedux;
    ...
    handleSaveUser = () => {
        ...
        if (action === CRUD_ACTIONS.CREATE) {
            ...
            this.props.editAUserRedux({
                ...
                positionId: this.state.position,
                avatar: this.state.avatar,
                mfaEnabled: this.state.role === 'R1' ? !!this.state.mfaEnabled : false
            })
        }
    }
    checkValidateInput = () => { // ham check dk neu chua nhap du cac truong -> bao loi
    ...
    render() {
        ...
        return (
            <div className="user-redux-container">
                <div className="title" >User Redux with Tran Quoc Duong</div>
                ...
                <div className="user-redux-body">
                    ...
                    <div className="container">
                        <div className="row">
                            ...
                            {this.state.action === CRUD_ACTIONS.EDIT && this.state.role === 'R1' && (
                                <div className="col-3 d-flex align-items-end">
                                    <div className="form-check">
                                        ...
                                        />
                                        <label className="form-check-label" htmlFor="mfaEnabledCheck">
                                            <FormattedMessage id="manage-user.mfa" />
                                        </label>
                                    </div>
                                </div>
                            )}
